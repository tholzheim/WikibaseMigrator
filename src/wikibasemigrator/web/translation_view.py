...
class TranslationView:
    ...
    async def translate(self, item_ids: list[str]):
        ...
        self.translations: ItemSetTranslationResult = await run.io_bound(self.migrator.translate_items_by_id, item_ids)
    ...
    def display_quickstatements(self):
        ...
        quickstatements = qs_generator.generate_items(self.translations.get_target_items())
    ...
    async def fetch_labels(self):
        ...
        source_labels_raw = await run.io_bound(
            ...
            item_ids=list(self.translations.get_mapping().keys()),
        ...
        target_labels_raw = await run.io_bound(
            callback=Query.get_item_label,
            item_ids=list(self.translations.get_mapping().values()),
        ...
        with item_container:
            try:
                TranslatedWikibaseItemWidget(
                    ...
                    translation_result=self.translations.get_translation_result_by_source_id(source_item_id),
                )
    ...
    def display_missing_properties(self):
        rows = []
        values = self.migrator.mapper.get_missing_property_mapings()
    ...
    def display_missing_items(self):
        rows = []
        values = self.migrator.mapper.get_missing_item_mapings()
        for entity_id in values:
            label = self.source_labels.get(entity_id, "")
            url = f"{self.profile.source.item_prefix}{entity_id}"
            rows.append(
    ...
    def display_applied_mappings(self):
        rows = []
        values = self.migrator.mapper.get_applied_mappings()
        for entity_id in values:
            label = self.source_labels.get(entity_id, "")
            url = f"{self.profile.source.item_prefix}{entity_id}"
            rows.append(
        ...
        with ui.element("div").classes("flex flex-col gap-2 w-full"):
            ui.label(f"Applied Mapping {len(values)}").classes("text-xl mx-auto")
            ui.aggrid(
                {
                    "columnDefs": [
                        {"headerName": "ID", "field": "id"},
